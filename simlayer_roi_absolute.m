...
%
% Use as
%   simlayer_roi_absolute(subjects(1), 1, [10 30], 20, 100)
% where the first argument is the subject info structure (from create_subjects),
% the second is the session number, the third is the frequency range, 
% the fourth is the dipole moment (nA), and the fifth is the white noise (rms fT).
% 
%   simlayer_roi_absolute(...,'param','value','param','value'...) allows
...
defaults = struct('surf_dir', 'd:\pred_coding\surf', 'mri_dir', 'd:\pred_coding\mri',...
    'out_path', '', 'sim_patch_size', 5, 'reconstruct_patch_size', 5);  %define default values
...
if length(params.out_path)==0
    params.out_path=fullfile('c:/layer_sim/ttest_results_abs',subj_info.subj_id,...
        num2str(session_num),sprintf('f%d_%d_dipolemoment%d_noise%d',invfoi(1),...
        invfoi(2),dipole_moment,white_noise));
end
...
% Load meshes
white_mesh=fullfile(params.surf_dir,sprintf('%s%s-synth',subj_info.subj_id,subj_info.birth_date),'surf','ds_white.hires.deformed.surf.gii');
pial_mesh=fullfile(params.surf_dir,sprintf('%s%s-synth',subj_info.subj_id,subj_info.birth_date),'surf','ds_pial.hires.deformed.surf.gii');
pialwhite_mesh=fullfile(params.surf_dir,sprintf('%s%s-synth',subj_info.subj_id,subj_info.birth_date),'surf','ds_white.hires.deformed-ds_pial.hires.deformed.surf.gii');
...
for simmeshind=1:length(simmeshes)
    ...
    for s=sims,
        ...
        matlabbatch{1}.spm.meeg.source.simulate.isinversion.setsources.dipmom = [dipole_moment params.sim_patch_size];
        matlabbatch{1}.spm.meeg.source.simulate.isinversion.setsources.locs = simpos;
        matlabbatch{1}.spm.meeg.source.simulate.isSNR.whitenoise = white_noise;
